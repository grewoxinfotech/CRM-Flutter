...
class BillCreateController extends GetxController {
  ...
  Future<void> _loadVendors() async {
    ...
    try {
      final vendorList = await _vendorService.getAllVendors();
      vendors.assignAll(vendorList);

      if (vendorList.isNotEmpty) {
        selectedVendorId.value = vendorList.first.id!;
        isGstEnabled.value = vendorList.first.taxNumber?.isNotEmpty ?? false;
        gstinController.text = vendorList.first.taxNumber ?? '';
      }
    } catch (e) {
      CrmSnackBar.showAwesomeSnackbar(
        title: 'Error',

  ...
  Future<void> getStages({String? stageType, String? clientId}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic>? stagesData;
        
        if (data['success'] == true) {
          if (data['data'] != null) {
            stagesData = data['data'] as List;
          } else if (data['message'] != null) {
            if (data['message'] is Map && data['message']['data'] != null) {
              stagesData = data['message']['data'] as List;
            }
          }
          ...
          CrmSnackBar.showAwesomeSnackbar(
            title: 'Error',
            message: data['message'] is String ? data['message'] : 'Failed to fetch stages',
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          if (stagesData != null && stagesData.isNotEmpty) {
            return stagesData.map((stage) => StageModel.fromJson(stage)).toList();
          }
        }
      }
      return [];
  ...
  Future<StageModel?> getDefaultStageForPipeline(String pipelineId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          if (data['data'] != null) {
            stageData = data['data'] as Map<String, dynamic>;
          } else if (data['message'] != null) {
            if (data['message'] is Map && data['message']['data'] != null) {
              stageData = data['message']['data'] as Map<String, dynamic>;
            }
          }
          
          if (stageData != null) {
            return StageModel.fromJson(stageData);
          }
        }
      }
      return null;

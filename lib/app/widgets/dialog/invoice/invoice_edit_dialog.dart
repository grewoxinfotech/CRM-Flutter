...
class InvoiceEditDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingController = TextEditingController(
      text: invoice.pendingAmount?.toString() ?? '0',
    );
    final issueDateController = TextEditingController(
      text: invoice.issueDate!.toIso8601String(),
    );
    final dueDateController = TextEditingController(
      text: invoice.dueDate!.toIso8601String(),
    );
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('Edit Invoice', style: Get.textTheme.titleLarge),
                            ...
                            : () async {
                              ...
                              final success = await controller.updateInvoice(
                                invoice.id!,
                                {
                                  ...
                                  'issueDate':
                                      issueDateController.text.split(
                                        'T',
                                      )[0], // Get only the date part
                                  'dueDate':
                                      dueDateController.text.split(
                                        'T',
                                      )[0], // Get only the date part
                                  ...
                                  'pendingAmount':
                                      double.tryParse(pendingController.text) ??
                                      0.0,
                                },
                              );
